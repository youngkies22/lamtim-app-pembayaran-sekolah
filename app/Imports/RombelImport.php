...
class RombelImport implements ToModel, WithHeadingRow, WithValidation, WithBatchInserts, WithChunkReading, SkipsOnFailure
{
    use SkipsFailures;
    ...
    public function model(array $row)
    {
        ...
        try {
            ...
            // Get jurusan ID if provided
            $idJurusan = null;
            if (!empty($row['jurusan'])) {
                $idJurusan = $this->getJurusanId($row['jurusan']);
            }

            return new LamtimRombel([
                'idSekolah' => $idSekolah,
                'idJurusan' => $idJurusan,
                'kode' => $row['kode'],
                'nama' => $row['nama'],
            ...
            Log::error('RombelImport error at row ' . $this->rowNumber, [
                'error' => $e->getMessage(),
                'row' => $row
            ]);
    ...
    /**
     * Get jurusan ID from nama
     */
    protected function getJurusanId(string $nama): ?string
    {
        if (isset($this->jurusanCache[$nama])) {
            return $this->jurusanCache[$nama];
        }

        $jurusan = LamtimJurusan::where('nama', 'like', "%{$nama}%")->first();
        if ($jurusan) {
            $this->jurusanCache[$nama] = $jurusan->id;
            return $jurusan->id;
        }
