...
const infoSections = [
  {
    title: 'Apa itu Jenis Pembayaran?',
    description: 'Jenis Pembayaran adalah kategori utama untuk mengelompokkan pembayaran berdasarkan jenisnya. Jenis ini akan digunakan saat membuat Master Pembayaran.',
    icon: markRaw(CreditCardIcon),
    items: [
      {
        name: 'SPP (Sumbangan Pembinaan Pendidikan)',
        description: 'Pembayaran rutin bulanan untuk operasional sekolah'
      },
      {
        name: 'PKL (Praktik Kerja Lapangan)',
        description: 'Pembayaran untuk kegiatan praktik kerja lapangan siswa'
      },
      {
        name: 'KI (Kunjungan Industri)',
        description: 'Pembayaran untuk kegiatan kunjungan ke industri/perusahaan'
      },
      {
        name: 'UKOM (Uji Kompetensi)',
        description: 'Pembayaran untuk uji kompetensi siswa'
      },
      {
        name: 'LAINNYA',
        description: 'Jenis pembayaran lainnya yang tidak termasuk kategori di atas'
      }
    ]
  },
  {
    title: 'Cara Menggunakan',
    description: 'Anda dapat menambahkan jenis pembayaran sesuai kebutuhan sekolah. Setelah dibuat, jenis ini akan muncul di dropdown saat membuat Master Pembayaran.',
...
const loadData = async () => {
  try {
    loading.value = true;
    const response = await masterDataAPI.jenisPembayaran.list({ search: searchQuery.value });
    tableData.value = response.data.data || response.data || [];
  } catch (err) {
    toastRef.value?.error('Gagal memuat data jenis pembayaran');
    tableData.value = [];
  } finally {
    loading.value = false;
  }
...
const handleEdit = async (id) => {
  try {
    ...
    const response = await masterDataAPI.jenisPembayaran.get(id);
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await masterDataAPI.jenisPembayaran.delete(deletingItem.value.id);
    showDeleteModal.value = false;
    deletingItem.value = null;
    toastRef.value?.success('Jenis pembayaran berhasil dihapus');
    loadData();
  } catch (err) {
    toastRef.value?.error(err.response?.data?.message || 'Gagal menghapus jenis pembayaran');
  } finally {
...
const handleSubmit = async () => {
  try {
    ...
    if (editingId.value) {
      await masterDataAPI.jenisPembayaran.update(editingId.value, payload);
      toastRef.value?.success('Jenis pembayaran berhasil diperbarui');
    } else {
      await masterDataAPI.jenisPembayaran.create(payload);
      toastRef.value?.success('Jenis pembayaran berhasil ditambahkan');
    }
