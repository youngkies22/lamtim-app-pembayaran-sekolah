<template>
  <Layout :active-menu="'Master Data'">
    <div class="space-y-6">
          ...
          <div>
            <h1 class="text-3xl font-bold text-white flex items-center gap-3">
              <div class="p-2 bg-white/20 rounded-xl backdrop-blur-sm">
                <RectangleStackIcon class="w-8 h-8 text-white" />
              </div>
              Data Tipe Pembayaran
              <button @click="showInfoModal = true" class="p-1.5 rounded-lg hover:bg-white/20 transition-colors group"
                title="Informasi Tipe Pembayaran">
                <InformationCircleIcon class="w-5 h-5 text-white group-hover:scale-110 transition-transform" />
              </button>
            </h1>
            <p class="mt-2 text-violet-100">Kelola tipe pembayaran untuk mengkategorikan pembayaran</p>
          </div>
          <button v-if="canCreateData" @click="openCreateModal"
            class="group relative inline-flex items-center gap-2 px-6 py-3 bg-white text-violet-600 font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
        <div class="relative">
          ...
          <input type="text" v-model="searchQuery" @input="debouncedSearch" placeholder="Cari tipe pembayaran..."
            class="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 dark:text-white transition-all duration-200" />
        </div>
      </div>
              ...
              <tr v-if="loading">
                <td colspan="6" class="px-6 py-16 text-center">
                  <div class="flex flex-col items-center justify-center gap-3">
                    <svg class="animate-spin h-10 w-10 text-violet-600" fill="none" viewBox="0 0 24 24">
    ...
    <!-- Form Modal -->
    <FormModal :show="showModal" :title="editingId ? 'Edit Tipe Pembayaran' : 'Tambah Tipe Pembayaran Baru'"
        ...
        <div class="space-y-5">
          <div>
            ...
            <input v-model="form.kode" type="text" required
              class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 dark:text-white"
          ...
          <div>
            ...
            <input v-model="form.nama" type="text" required
              class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 dark:text-white"
              placeholder="Nama lengkap tipe pembayaran" />
          </div>
          <div>
            ...
            <textarea v-model="form.keterangan" rows="3"
              class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 dark:text-white"
              placeholder="Keterangan tipe pembayaran"></textarea>
          </div>
    ...
    <ConfirmModal :show="showDeleteModal" type="danger" title="Hapus Tipe Pembayaran"
      :message="`Apakah Anda yakin ingin menghapus tipe pembayaran '${deletingItem?.nama || ''}'?`"
      confirm-text="Ya, Hapus" :loading="deleteLoading" @confirm="confirmDelete" @cancel="showDeleteModal = false" />

    <!-- Info Modal -->
    <InfoModal :show="showInfoModal" title="Informasi Tipe Pembayaran"
      subtitle="Pelajari lebih lanjut tentang tipe pembayaran yang bisa Anda isi" :sections="infoSections"
...
const infoSections = [
  {
    title: 'Apa itu Tipe Pembayaran?',
    description: 'Tipe Pembayaran adalah kategori untuk mengelompokkan metode pembayaran yang digunakan siswa. Tipe ini akan digunakan saat melakukan transaksi pembayaran.',
    ...
    items: [
      ...
      {
        name: 'QRIS',
        description: 'Pembayaran menggunakan QR Code (QRIS)'
      },
      {
        name: 'E-WALLET',
        description: 'Pembayaran menggunakan dompet digital (OVO, GoPay, Dana, dll)'
      },
      {
        name: 'LAINNYA',
        description: 'Tipe pembayaran lainnya yang tidak termasuk kategori di atas'
      }
    ]
  },
  {
    title: 'Cara Menggunakan',
    description: 'Anda dapat menambahkan tipe pembayaran sesuai kebutuhan sekolah. Setelah dibuat, tipe ini akan muncul di dropdown saat melakukan transaksi pembayaran.',
...
const loadData = async () => {
  try {
    loading.value = true;
    const response = await masterDataAPI.tipePembayaran.list({ search: searchQuery.value });
    tableData.value = response.data.data || response.data || [];
  } catch (err) {
    toastRef.value?.error('Gagal memuat data tipe pembayaran');
    tableData.value = [];
  } finally {
    loading.value = false;
  }
...
const handleEdit = async (id) => {
  try {
    ...
    const response = await masterDataAPI.tipePembayaran.get(id);
    const tipe = response.data.data;
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await masterDataAPI.tipePembayaran.delete(deletingItem.value.id);
    showDeleteModal.value = false;
    deletingItem.value = null;
    toastRef.value?.success('Tipe pembayaran berhasil dihapus');
    loadData();
  } catch (err) {
    toastRef.value?.error(err.response?.data?.message || 'Gagal menghapus tipe pembayaran');
  } finally {
...
const handleSubmit = async () => {
  try {
    ...
    if (editingId.value) {
      await masterDataAPI.tipePembayaran.update(editingId.value, payload);
      toastRef.value?.success('Tipe pembayaran berhasil diperbarui');
    } else {
      await masterDataAPI.tipePembayaran.create(payload);
      toastRef.value?.success('Tipe pembayaran berhasil ditambahkan');
    }
