<template>
  <Layout :active-menu="'Master Data'">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div class="relative">
          ...
          <input
            ...
            :placeholder="`Cari ${labelJurusan.toLowerCase()}...`"
            class="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:text-white transition-all duration-200"
          />
        </div>
      </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gray-50 dark:bg-gray-900/50">
                ...
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nama {{ labelJurusan }}</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Kode Sekolah</th>
    ...
    <FormModal
      :show="showModal"
      :title="editingId ? `Edit ${labelJurusan}` : `Tambah ${labelJurusan} Baru`"
        ...
        <div class="space-y-5">
          <div>
            ...
            <input
              ...
              :placeholder="`Kode ${labelJurusan.toLowerCase()}`"
            />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Nama {{ labelJurusan }} *</label>
            <input
              v-model="form.nama"
              type="text"
              required
              class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 dark:text-white"
              :placeholder="`Nama lengkap ${labelJurusan.toLowerCase()}`"
            />
          </div>
          <div>
    ...
    <ConfirmModal
      ...
      type="danger"
      :title="`Hapus ${labelJurusan}`"
      :message="`Apakah Anda yakin ingin menghapus ${labelJurusan.toLowerCase()} '${deletingItem?.nama || ''}'?`"
...
const loadData = async (force = false) => {
  try {
    ...
    const response = await masterDataAPI.jurusan.list(params);
    ...
    toastRef.value?.error(`Gagal memuat data ${labelJurusan.value.toLowerCase()}`);
    tableData.value = [];
  } finally {
    loading.value = false;
  }
...
const handleEdit = async (item) => {
  try {
    ...
    const response = await masterDataAPI.jurusan.get(item.id);
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await masterDataAPI.jurusan.delete(deletingItem.value.id);
    showDeleteModal.value = false;
    deletingItem.value = null;
    toastRef.value?.success(`${labelJurusan.value} berhasil dihapus`);
    ...
    toastRef.value?.error(err.response?.data?.message || `Gagal menghapus ${labelJurusan.value.toLowerCase()}`);
  } finally {
...
const handleSubmit = async () => {
  try {
    ...
    if (editingId.value) {
      await masterDataAPI.jurusan.update(editingId.value, payload);
      toastRef.value?.success(`${labelJurusan.value} berhasil diperbarui`);
    } else {
      await masterDataAPI.jurusan.create(payload);
      toastRef.value?.success(`${labelJurusan.value} berhasil ditambahkan`);
    }
