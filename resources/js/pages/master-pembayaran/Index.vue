<template>
  <Layout :active-menu="'Payments'">
    <div class="space-y-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div class="relative">
            <MagnifyingGlassIcon class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none w-5 h-5 text-gray-400" />
            <input
              type="text"
              v-model="searchQuery"
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gray-50 dark:bg-gray-900/50">
                ...
                <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Cicilan</th>
              ...
              <tr v-else v-for="(item, index) in tableData" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                ...
                <td class="px-6 py-4 text-center">
                  <span :class="[
                    ...
                    <span :class="['w-1.5 h-1.5 rounded-full mr-1.5', item.isActive ? 'bg-green-500' : 'bg-red-500']"></span>
    ...
    <FormModal
      :show="showModal"
      :title="editingId ? 'Edit Master Pembayaran' : 'Tambah Master Pembayaran Baru'"
      ...
      <template #form>
        <div class="space-y-5">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Kode <span class="text-xs text-gray-500">(Otomatis)</span></label>
              <div class="relative">
                <input
                  v-model="form.kode"
                  type="text"
          ...
          <div v-if="form.isCicilan" class="grid grid-cols-2 gap-4">
            <div>
              ...
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">Rp</span>
                <input
                  v-model.number="form.minCicilan"
                  type="number"
                  min="0"
                  step="0.01"
                  class="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-emerald-500 dark:text-white"
            ...
            <div>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Jumlah Bulan (untuk SPP)</label>
              <input
...
const generateKode = () => {
  ...
  // Generate timestamp (format: YYYYMMDDHHmmss - 14 digit)
