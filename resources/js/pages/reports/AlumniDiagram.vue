    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3">
          ...
          <div class="relative w-full sm:w-64">
            <select v-model="filterSekolah" @change="handleSchoolChange"
              class="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white appearance-none font-medium">
          ...
          <button v-if="hasData" @click="refreshData"
            class="flex items-center gap-2 px-4 py-2.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400 rounded-xl transition-all font-semibold text-sm whitespace-nowrap"
            :disabled="loading">
      ...
      <div v-if="!hasData && !loading"
        ...
        <div class="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/20 rounded-full flex items-center justify-center mb-4">
          <AdjustmentsHorizontalIcon class="w-10 h-10 text-indigo-500" />
        </div>
        ...
        <p class="text-gray-500 dark:text-gray-400 mt-2 max-w-sm">
          Pilih <b>Sekolah</b> dan <b>Tahun Angkatan</b> pada opsi di atas untuk menampilkan diagram analisa alumni.
        </p>
      </div>
      ...
      <template v-if="hasData">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div
            ...
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400">Total Alumni</p>
        ...
        <!-- Charts Grid -->
        <div v-if="byYear.length > 0" class="space-y-6">
          <!-- Row 1: Stacked Bar (2/3) + Donut (1/3) -->
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2">
              <AlumniStackedBarChart :data="byYear" :loading="loading" :is-dark="isDark" />
            </div>
            <div class="lg:col-span-1">
              <AlumniDonutChart :summary="summary" :loading="loading" :is-dark="isDark" />
            </div>
          </div>

          <!-- Row 2: Horizontal Bar (1/2) + Line Chart (1/2) -->
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <AlumniHorizontalBarChart :data="byYear" :loading="loading" :is-dark="isDark" />
            <AlumniTrendLineChart :data="byYear" :loading="loading" :is-dark="isDark" />
          </div>
        </div>
        ...
        <div v-else
          ...
          <p class="text-gray-500 dark:text-gray-400">
            Tidak ada data alumni yang ditemukan{{ filterYear && filterYear !== 'all' ? ' untuk Angkatan ' + filterYear : '' }}.
          </p>
        </div>
      </template>
    </div>
...
import AlumniDonutChart from '../../components/charts/AlumniDonutChart.vue';
import AlumniHorizontalBarChart from '../../components/charts/AlumniHorizontalBarChart.vue';
import AlumniTrendLineChart from '../../components/charts/AlumniTrendLineChart.vue';
import {
...
const loadData = async (options = {}) => {
  try {
    ...
    console.error('Failed to load alumni diagram data:', error);
  } finally {
    loading.value = false;
  }
};
